...
";
    ...
    public static string BuildConspectPrompt(string title, IEnumerable<string> questions, string lang = "Русский")
    {
        var q = string.Join("\n- ", questions ?? Array.Empty<string>());

        return $@"
Ты — учитель средней школы. Создай подробный, структурированный и понятный конспект урока.

ЯЗЫК ВЫВОДА — СТРОГО {lang}.
Запрещено использовать английский язык. Никаких английских слов, предложений или выражений.
...
Формат конспекта:
- 6–10 разделов с заголовками,
- ключевые определения, короткие примеры,
- типичные ошибки,
- маленькие вопросы для самопроверки по ходу,
- в конце 3–5 конкретных советов по теме.

Выводи ТОЛЬКО текст конспекта, без JSON, без пояснений, без фраз вроде ""Вот ваш конспект"".
";
    }
}